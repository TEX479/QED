...
class GUI():
    def __init__(self, texts = {"o":"", "p":"", "v":"", "e":""}, chunk = 16, KEY = ["",""], debug = False) -> None:
        self.texts = texts
        self.KEY = KEY
        self.chunk = chunk
        self.debug = debug
        ...
        if system == "Linux":
            ...
            exit()
        gui_version = (os.path.abspath(__file__)).split(self.präfix)[-1][8:-3]

        if self.debug: print(os.path.abspath(__file__))
        path = self.präfix.join(i for i in (os.path.abspath(__file__)).split(self.präfix)[:-1])
