...
class Verschlüsselung():
    ...
    def _generate_key_data(self, length):
        '''
        returns a random key; len(key) == length
        NOTE: nicht alle schlüssellängen sind vom Programm supportet.
        '''
        key = ""
    ...
    def _generate_key_file(self, key, output):
        '''
        stores the var 'key' to the 'output' file
        '''
        with open(output, "w") as f:
            f.write(key)
    
    def generate_key(self, length, output):
        '''
        generiert einen zufälligen schlüssel der länge 'length' und speichert diesen in der datei 'output'
        '''
        key = self._generate_key_data(length)
        self._generate_key_file(key, output)
