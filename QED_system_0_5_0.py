...
class Verschlüsselung():
    def __init__(self, chunk = 16, debug = True, debug_c = False, cube_field_data_size = 1) -> None:
        """
    ...
    def _cube_int_to_moves(self, integer, cube_dimensions, encrypt):
        ...
        if not(encrypt):
            step_array.reverse()
        if self.debug: print(len(step_array))
    ...
    def cube(self, text, key_m_cube, cube_dimensions=0, cube_field_data_size=0, encryption=True):
        ...
        for i in step_array:
            cube.rotate(i[0], i[1], i[2])
        
        text_scrambled = self._cube_get_data(cube.cube.copy())
...
def run_test(x= (0,1)):
    ...
    for i in range(repeat):
        ...
        #if debug: print(test)
        if test == x.entschlüsseln(x.verschlüsseln(text=test, KEY=key), key): y+=1#print("Y")#Y+=1
        #else: print("N")#N+=1; N_list.append((test, key))
    #print("Y : ", y, " | ", "N : ", repeat-y)
    t = time.time() - t
    l = " "*(18 -len(str(t)))
    p = f"{y/repeat}  {t} {l} {nr}"
    print(p)
...
if __name__ == "__main__":
    ...
    t = time.time()
    with Pool(4) as p:
        p.map(run_test, [(0, 100), (1, 100), (2, 100), (3, 100)])
    t = time.time() - t
    print(t)
    #print("Y:", Y, "|", "N:", R-Y, "|", "D:", t)
    #print(N_list)
