...
class Verschlüsselung():
    ...
    def get_key(self, KEY:str, text=""):
        ...
        if key_mix>1: key_mix = int(key_mix%len(self.hilfsfunktionen.BitToStr(text, self.chunk)))
        else: key_mix = int((key_mix*len(self.hilfsfunktionen.BitToStr(text, self.chunk)))%len(self.hilfsfunktionen.BitToStr(text, self.chunk)))

        key_start = int(S3[0]%len(self.hilfsfunktionen.BitToStr(text, self.chunk))) if S3[0]>=0 else int((S3[0]*len(self.hilfsfunktionen.BitToStr(text, self.chunk)))%len(self.hilfsfunktionen.BitToStr(text, self.chunk)))
    ...
    def get_key_m_cube(self, key_normal:list, key_start:int) -> int:# parallel? + Cube?
        g=300
        key_m_cube = ""
        while key_start > 0:
            if key_start - key_normal[0] > 0:
                ...
                key_start = 0
        key_m_cube = int("".join(i for i in key_normal))
        #evtl. umwandeln in andere Systeme
        key_m_cube = self.hilfsfunktionen.int2anybase(number=key_m_cube,base=5)
        key_m_cube = int("".join(key_m_cube[i]+key_m_cube[i+1] for i in range(0, len(key_m_cube)//2, 2)))

        while len(str(key_m_cube)) > math.log((5**(2*math.log(18**g,10))),10):
            key_m_cube = self.hilfsfunktionen.int2anybase(number=key_m_cube, base=5)# 5 -> ?
    ...
    def structure_mix_letter(self, way:bool, key:int, full_text_:str) -> str:
        ...
        """
        chunk = 4 #?
        full_text = list(self.hilfsfunktionen.BitToStr(full_text_, chunk))
        #erzeugung und anpassung des Schlüssels + Text
        key_ = key
        go_on = True
        text_end = False
        ...
        for i in range(len(key_mix)): key_mix[i] = int(self.hilfsfunktionen.StrToBit(key_mix[i], lenght=chunk),2) + 1
        s2 = key_mix.copy()
        s2.sort()
        ...
        return "".join(i for i in self.hilfsfunktionen.StrToBit(full_text, lenght=chunk))
    ...
    def cube(self, text, key_m_cube, cube_dimensions=0, cube_field_data_size=0, encryption=True, cores=0):
        ...
        text_scrambled.sort()
        text_scrambled = "".join(i[1] for i in text_scrambled)
