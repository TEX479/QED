...
class Verschlüsselung():
    ...
    def cube(self, text, key_m_cube, cube_dimensions=0, cube_field_data_size=0, encryption=True):
        ...
        for i in text_formatted:
            text_scrambled += "".join(i2 for i2 in self._mix_letter(text=i, key=key_m_cube_2, way=not(encryption)))# eigentlich nur encryption und nicht not(encryption), aber egal
