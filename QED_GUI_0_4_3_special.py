...
def find_number(string) -> list:
    """
    findet Zahlen in einem String
    """
    go_on = True
    position = 0
    found = []
    while go_on:
        try:
            ...
            while go_on_2:
                try:
                    integer=10*integer+int(string[position])
                    position += 1
                except ValueError:
                    if string[position] == ".":
                        go_on_3 = True
                        integer_2 = 0
                        position += 1
                        while go_on_3 and position < len(string):
                            try:
                                integer_2=10*integer_2+int(string[position])
                                position += 1
                            except:
                                go_on_3 = False
                        go_on_2 = False
                        found.append(float(str(integer)+"."+str(integer_2)))
                    else:
                        go_on_2 = False
...
class GUI():
    ...
    def create_gui(self, version_index, gui_version):
        ...
        self.anz_cores_txt = tkinter.Text(master=self.top_btns, background = "#ffffff", fg = "#000000", width=20, height=1)
        self.anz_cores_txt.grid(row=0, column=3)
    ...
    def start(self):
        if (self.texts["o"] != "") and (self.KEY[0] != ""):
            ...
            duration = time.time()
            v = Verschlüsselung(chunk = self.chunk, debug = False, cores = (find_number(self.anz_cores_txt.get("1.0", tkinter.END)))[0])
