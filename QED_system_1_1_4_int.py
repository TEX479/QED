...
class Verschlüsselung():
    ...
    def get_key(self, KEY:str) -> dict[str, list[int] | int]:
        """
        ...
        if len(S3) == 1:
            S3 = [int(S[:len(S)//2], 2), int(S[len(S)//2:], 2)] # type: ignore <- overwriting a constant is forbidden, but that is how bg95ght87 designed the code, and i am just typehinting here.

        for i in range(1, len(S3)):
            key_normal.append(round(S3[i])+1)
        if len(S3) == 2:
            key_normal.append(int(S3[1])+1)

        if key_mix>=1:
            key_mix = int(key_mix%math.ceil(self.l/self.chunk))
        else:
            key_mix = int(key_mix*math.ceil(self.l/self.chunk))

        key_start = int(S3[0]%math.ceil(self.l/self.chunk)) if S3[0]>=1 else int(S3[0]*math.ceil(self.l/self.chunk))
        if key_start == 0:
            key_start=math.ceil(self.l/self.chunk)-1
    ...
    def get_key_m_cube(self, key_normal:list[int], key_start:int, g:int=250) -> int:
        ...
        while len(key_m_cube) < g:
            key_m_cube_int = int("".join(str(i) for i in key_m_cube))
            key_m_cube_floats = hilfsfunktionen.int2anybase2(number=key_m_cube_int, base=1.7)
            key_m_cube_string = "".join(str(i*10).split(".")[0] for i in key_m_cube_floats)
            key_m_cube: list[int] = [int(i) for i in key_m_cube_string]
        ...
        key_m_cube = hilfsfunktionen.int2anybase(number=key_m_cube_int,base=5)
        key_m_cube = [key_m_cube[i]+key_m_cube[i+1] for i in range(0, len(key_m_cube)//2, 2)]# + %10 ?
        key_m_cube = hilfsfunktionen.anybase2anybase(key_m_cube, 9, 10)    
        
        key_m_cube_int = int("".join(str(i) for i in key_m_cube))

        return key_m_cube_int

    def entschlüsseln(self, text_input:str, KEY: str) -> str:
        """
        `text_input` is a string consisting of "1"s and "0"s
        """
        """
        ...
        way=True
        """
        self.l = len(text_input)
        text = int(text_input,2)
        
        keys = self.get_key(KEY=KEY)
        if self.debug:
            print("\n--- ENTSCHLÜSSELN ---\n")
        if self.debug:
            print(f"original:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_0_original.txt", "wb") as f:
                f.write(bytes(BitToInt(f"{text:0{self.l}b}")))


        text = self.mix_letter(way=True,text=text, key=keys["m"]) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach mix_letter:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
        ...
        text_part = self.VER_1(way=True, text=text_part, key=keys["n"].copy(), l2=l2) # type: ignore # TODO: fix the "keys" situation
        text_part = f"{text_part:0{l2}b}"[::-1]
        text_ = f"{text:0{self.l}b}"[l2:]
        text = int(text_part + text_,2)#erw1
        if self.debug:
            print(f"nach erw1:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_2_nach_m1_2.txt", "wb") as f:
                f.write(bytes(BitToInt(f"{text:0{self.l}b}")))


        text = self.cube(text=text, key_m_cube=keys["c"], encryption=not(True)) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach cube:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_3_nach_cube.txt", "wb") as f:
                f.write(bytes(BitToInt(f"{text:0{self.l}b}")))


        text = self.VER_1(way=True, text=text, key=keys["n"].copy(), l2=self.l) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach m1:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
    ...
    def verschlüsseln(self, text_input: str, KEY: str) -> str:
        """
        `text` is a string consisting of "1"s and "0"s
        """
        """
        ...
        text = self.VER_1(way=False, text=text, key=keys["n"].copy(), l2=self.l) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach m1:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_4_nach_m1.txt", "wb") as f:
        ...
        text = self.cube(text=text, key_m_cube=keys["c"], encryption=not(False)) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach cube:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_3_nach_cube.txt", "wb") as f:
        ...
        l2: int = keys["s"]*self.chunk # type: ignore # TODO: fix the "keys" situation
        text_part = f"{text:0{self.l}b}"[:l2]
        text_part = int(text_part[::-1],2)
        text_part = self.VER_1(way=False, text=text_part, key=keys["n"].copy(), l2=l2) # type: ignore # TODO: fix the "keys" situation
        text_part = f"{text_part:0{l2}b}"[::-1]
        text_ = f"{text:0{self.l}b}"[l2:]
        text = int(text_part + text_,2)#erw1
        if self.debug:
        ...
        text = self.mix_letter(way=False,text=text, key=keys["m"]) # type: ignore # TODO: fix the "keys" situation
        if self.debug:
            print(f"nach mix_letter:\t {text:0{self.l}b} \nLänge: {self.l}\n")
        if self.debug_f:
            with open("ENT_1_nach_mix_letter.txt", "wb") as f:
    ...
    def _mix_letter(self, way: bool, text:int, key: list[int], l2: int, chunk: int = 4) -> int:
        """
    ...
    def mix_letter(self, way:bool, key:int, text:int, chunk: int = 4) -> int:
        """
        ...
        if last_number != -1:
            key_mix_copy.append(last_number)
        
        for i in range(len(key_mix)):
            key_mix[i] += 1
        s2 = key_mix.copy()
        s2.sort()
        for i in range(len(s2)):# alle Zahel aufeinanderfolgend 1.. bsp. 5 1 3 -> 3 1 2 
    ...
    def VER_1(self, way: bool, text: int, key: list[int], l2: int) -> int:
        ...
        text_r = text
        key_ = 0
        chunks: int = math.ceil(l2/self.chunk)
    ...
    class cube_class():
        def __init__(self, dimensions:int=3, debug:bool=False):
            self.dimensions = dimensions
            self.debug = debug

            self.cube: list[list[list[str]]] = []
            for i0 in range(6):
                self.cube.append([])
                for i1 in range(self.dimensions):
                    self.cube[i0].append([])
                    for _ in range(self.dimensions):
                        self.cube[i0][i1].append("")
            if self.debug:
        ...
        def rotate_array(self, arr: list[list[Any]], rotation:int=1) -> list[list[Any]]:
            ...
            for i in range((rotation % 4) + 4):
                ...
                for i in range(len(h)):
                    h[i] = list(h[i]) # type: ignore
                arr = h # type: ignore
            return arr
        
        def rotate(self, axis: Literal["x", "y", "z"], plane: int, rotation: int):
            if axis == "z":
                rotation = 4 - rotation
            for _ in range(rotation % 4):
                self._rotate(axis, plane)

        def _rotate(self, axis: Literal["x", "y", "z"], plane: int):
    ...
    def _cube_int_to_moves(self, integer: int, cube_dimensions: int, encrypt: bool) -> list[tuple[Literal['x', 'y', 'z'], int, Literal[-1, 1]]]:
        '''
        ...
        '''
        step_array: list[tuple[Literal['x', 'y', 'z'], int, Literal[-1, 1]]] = []
        seed: list[int] = hilfsfunktionen.int2anybase(integer, cube_dimensions*3)
        ...
        '''
        _AXIS: list[Literal['x', 'y', 'z']] = ["x", "y", "z"]
        for i in seed:
            axis = _AXIS[i%3]
            plane = i // 3
            if encrypt:
    ...
    def cube(self, text: int, key_m_cube: int, cube_dimensions: int=0, cube_field_data_size: int=0, encryption: bool=True) -> int:
        ...
        if (self.l >= (20*20*6)) and encryption:
            cube_field_data_size_local = self.l // (20*20*6)
        ...
        key_m_cube_2 = int("".join(f"{i:0{8}b}" for i in range(1, 217, 1)),2)
        for i2 in step_array:
            key_m_cube_2 = self._mix_letter(text=key_m_cube_2, key=quick_rotate[i2], way=False, chunk=8, l2=216*8)
        
        key_m_cube_2 = f"{key_m_cube_2:0{216*8}b}"
        key_m_cube_2 = [int(key_m_cube_2[i*8:(i+1)*8],2) for i in range(216)]

        text_scrambled = self._mix_letter(text=text, key=key_m_cube_2, way=encryption, chunk=1, l2=self.l)# encryption -> eventuell Schlüssel umdrehen, da self._cube_int_to_moves(key_m_cube, 6, True)
        #text_scrambled = text # nur für tests da!
        
        if (self.l >= (20*20*6)) and not(encryption):
            cube_field_data_size_local = self.l // (20*20*6)
            key_m_cube_big = hilfsfunktionen.int2anybase(key_m_cube, 42)
            key_m_cube_big = int(str(self.get_key_m_cube(key_m_cube_big, 343, 1000)))
    ...
    def cube_big(self, text: int, key_m_cube: int, cube_dimensions: int=0, cube_field_data_size: int=0, encryption: bool=True) -> int:
        '''
        mappt <text> in <cube_field_data_size> großen stücken auf die oberfläche eines rubics-cube
        mit <cube_dimensions> "flächen" (das entscheidet also obs ein 3x3x3, 4x4x4, ... ist)
        ...
        lshift = cube_dimensions**2 *6 *cube_field_data_size
        text_formatted = text & ((1<<(lshift))-1)
        text_formatted = f"{text_formatted:0{lshift}b}"
        ...
        text_scrambled = ((text >> lshift)<<lshift) + int(self._cube_get_data(cube.cube.copy()),2)
        return text_scrambled
...
def run_test(l1: int, l2: int) -> float:
    ...
    #if debug: print(test)
    t = time.time()
    encrypted = x.verschlüsseln(text_input=test, KEY=key)
    decrypted = x.entschlüsseln(encrypted, key)
    t = time.time() - t
    if test == decrypted:
        Y+=1 # type: ignore # i know this is forbidden, but it is just bg95ght87's design. not fixing that in "type hinting"
    else:
        N+=1#; N_list.append((test, key)) # type: ignore # i know this is forbidden, but it is just bg95ght87's design. not fixing that in "type hinting"
    return t

def run_test_multiprocessing(data: tuple[int, int, int, int]):
    print("+")
    ...
    for i in range(von, bis+1, s):
        ...
        for _ in range(r):
            ...
            #if debug: print(test)
            encrypted = x.verschlüsseln(text_input=test, KEY=key)
            decrypted = x.entschlüsseln(text_input=encrypted, KEY=key)
            if test == decrypted:
                Y+=1 # type: ignore # i know this is forbidden, but it is just bg95ght87's design. not fixing that in "type hinting"
            else:
                N+=1#; N_list.append((test, key)) # type: ignore # i know this is forbidden, but it is just bg95ght87's design. not fixing that in "type hinting"
...
def test1():
    ...
    for l1 in range(start, stop+1, step):
        ...
        #t = time.time() - t
        print("\nY:", Y, "|", "N:", N, "|", "D:", t)
        Y, N = 0, 0 # type: ignore # i know this is forbidden, but it is just bg95ght87's design. not fixing that in "type hinting"
        data.append(t/r)
...
def test2():
    ...
    stop = 2000
    step = 100
    work = [(anz, start, stop, step) for _ in range(cores)]
    print("begin\n")
