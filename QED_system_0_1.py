...
class Verschlüsselung():
    ...
    def get_key(self, KEY:str, text=""):
        ...
        else: key_mix = int(key_mix*len(BitToStr(text, self.chunk)))

        key_start = S3[0] if S3[0]>=0 else S3[0]*len(BitToStr(text, self.chunk))
    ...
    def entschlüsseln(self, text="", KEY = "") -> str:
        ...
        text = (self.structure_m1(way=True, text=text[0:keys["s"]][::-1], key=keys["n"]))[::-1] + text[keys["s"]:]#erw1
        if self.debug: print("nach erw1:       ", text, "\n")
        text = self.structure_m1(way=True, text=text, key=keys["n"])
    ...
    def verschlüsseln(self, text="", KEY = "") -> str:
        ...
        if self.debug: print("original:        ", text, "\n")
        text = self.structure_m1(way=False, text=text, key=keys["n"])
        if self.debug: print("nach m1:         ", text, "\n")
        text = (self.structure_m1(way=False, text=text[0:keys["s"]][::-1], key=keys["n"]))[::-1] + text[keys["s"]:]#erw1
        if self.debug: print("nach erw1:       ", text, "\n")
        text = (self.structure_mix_letter(way=True, full_text_=text, key=keys["m"]))
        if self.debug: print("nach mix_letter: ", text, "\n")
            ...
            if i%2 == 0:
                for i2 in range(key[i]):
                    ...
                    if pos < 0:
                        go_on = False
                        break
                    part = IntToBit((int(part, 2)^int(text[pos], 2)), self.chunk)
            else: 
                pos-=key[i]
                if pos < 0:
                    go_on = False
    ...
    def structure_mix_letter(self, way:bool, key:int, full_text_:str) -> str:#VERBESSERN!!!
        """
    ...
    def structure_m1(self, way, text:str, key:list) -> str:# Verschnellern!
        """
...
def run_test():
    ...
    test = ""
    key = ""
    for i in range(64): test += str(randint(0, 1))
