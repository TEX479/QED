...
class Verschlüsselung():
    ...
    def VER_1(self, key:list, part:str) -> str:
        """
    ...
    def structure_m1(self, way, text:str, key:list) -> str:
        ...
        if way:#ENT
            for i in range(len(text)//self.chunk):
                key_str = (self.VER_1(part=text[i*self.chunk : (i+1)*self.chunk], key=key))[:len(text)-i*self.chunk+1]
                text = text[: (i+1)*self.chunk] + IntToBit(x = (int(text[(i+1)*self.chunk : len(key_str) + (i+1)*self.chunk + 1])^int(key_str)))
        else:#VER
            for i in range(len(text)//self.chunk):
                key_str = (self.VER_1(part=text_[i*self.chunk : (i+1)*self.chunk], key=key))[:len(text)-i*self.chunk+1]
                text = text[: (i+1)*self.chunk] + IntToBit(x = (int(text_[(i+1)*self.chunk : len(key_str) + (i+1)*self.chunk + 1])^int(key_str)))
