...
class Verschlüsselung():
    ...
    def get_key(self, KEY:str, text=""):
        ...
        for i in range(len(KEY_copy)//key_len):
            KEY_arr_byte.append(KEY_copy[i*key_len:(i+1)*key_len])
        if len(KEY_copy)%key_len != 0:
            KEY_arr_byte.append(KEY_copy[(len(KEY_copy)//key_len)*key_len:])
        if len(KEY_arr_byte[-1])==1: KEY_arr_byte[-1] += "0"
        if self.debug: print("KEY_arr_byte: ",KEY_arr_byte)

        while current_position < len(KEY_arr_byte):
            if (current_position < len(KEY_arr_byte)) and (KEY_arr_byte[current_position][0] == "0"):
                if int(KEY_arr_byte[current_position][1:]) != 0:
                    current_vaule = KEY_arr_byte[current_position][1:]#-----------------------------------------------------------------------------
                    current_position += 1
                    while (current_position < len(KEY_arr_byte)) and (KEY_arr_byte[current_position][0] == "0"):
                        current_vaule += KEY_arr_byte[current_position][1:]#---------------------------------------------------------------------
                        current_position += 1
                    current_vaule = int(current_vaule, 2)
                    ...
                    while (current_position < len(KEY_arr_byte)) and (KEY_arr_byte[current_position][0] == "0"):
                        current_vaule += KEY_arr_byte[current_position][1:]
                        current_position += 1
                    current_vaule = float(f"0.{int(current_vaule, 2)}")
                KEY_arr_int.append(current_vaule)
                key_mix += current_vaule


            if (current_position < len(KEY_arr_byte)) and (KEY_arr_byte[current_position][0] == "1"):
                if int(KEY_arr_byte[current_position][1:]) != 0:
                    current_vaule = KEY_arr_byte[current_position][1:]#-----------------------------------------------------------------------------
                    current_position += 1
                    while (current_position < len(KEY_arr_byte)) and (KEY_arr_byte[current_position][0] == "1"):
                        current_vaule += KEY_arr_byte[current_position][1:]#---------------------------------------------------------------------
                        current_position += 1
                    current_vaule = int(current_vaule, 2)
        ...
        if len(KEY_arr_int) == 1: KEY_arr_int = [int(KEY_copy[:len(KEY_copy)//2], 2), int(KEY_copy[len(KEY_copy)//2:], 2)]

        for i in range(1, len(KEY_arr_int)): key_normal.append(round(KEY_arr_int[i])+1)
        if len(KEY_arr_int) == 2: key_normal.append(int(KEY_arr_int[1]))

        if key_mix>1: key_mix = int(key_mix%(len(text)//self.chunk))
        else: key_mix = int((key_mix*(len(text)//self.chunk))%(len(text)//self.chunk))

        key_start = int(KEY_arr_int[0]%(len(text)//self.chunk)) if KEY_arr_int[0]>=0 else int((KEY_arr_int[0]*(len(text)//self.chunk))%(len(text)//self.chunk))
        
        if self.debug: print("KEY_arr_int: ",KEY_arr_int)
    ...
    def generate_key(length):
        """
        erstellt ein str mit length zeichen/bytes
        """
        key = ""
