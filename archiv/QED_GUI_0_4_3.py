...
class GUI():
    ...
    def create_gui(self, version_index, gui_version):
        ...
        self.anz_cores_txt = tkinter.Text(master=self.top_btns, background = "#ffffff", fg = "#000000", width=5, height=1)
    ...
    def start(self):
        if (self.texts["o"] != "") and (self.KEY[0] != ""):
            ...
            duration = time.time()
            v = Verschlüsselung(chunk = self.chunk, debug = False, cores = int(self.anz_cores_txt.get("1.0", tkinter.END)))
