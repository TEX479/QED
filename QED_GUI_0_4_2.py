...
class GUI():
    ...
    def create_gui(self, version_index, gui_version):
        ...
        self.anz_cores_lbl = tkinter.Label(master=self.top_btns, background = "#ffffff", fg = "#000000", text="Azahl Kerne: ")
        self.anz_cores_lbl.grid(row=0, column=2)
        self.options_list = [i+1 for i in range(10)]
        self.value_inside_c = tkinter.StringVar(self.top_btns)
        self.value_inside_c.set("Select an Option")
        self.question_menu = tkinter.OptionMenu(self.top_btns, self.value_inside_c, *self.options_list)
        self.question_menu.grid(row=0, column=3)
    ...
    def start(self):
        if (self.texts["o"] != "") and (self.KEY[0] != ""):
            ...
            v = Verschlüsselung(chunk = self.chunk, debug = False, cores = int(self.value_inside_c.get()))
            ...
            if self.debug: print("V: ", self.texts["v"], "\n", "E:", self.texts["e"])
            ...
            self.TEXT_v_sct.insert(tkinter.INSERT, BitToStr(self.texts["v"]))
            self.TEXT_v_sct.configure(state="disabled")

            self.TEXT_e_sct.configure(state="normal")
            self.TEXT_e_sct.delete("1.0", tkinter.END)
            self.TEXT_e_sct.insert(tkinter.INSERT, BitToStr(self.texts["e"]))
    ...
    def open_file_f(self, w=None):
        if w == "k":
            ...
            self.TEXT_o_sct.insert(tkinter.INSERT, BitToStr(self.texts["o"]))
            self.TEXT_o_sct.configure(state="disabled")
            self.TEXT_o_lbl_p.configure(text=str(self.texts["p"]))

    def save_key(self):
        filepath = askopenfilename(filetypes=[("All Files", "*"), ("Textfiles", "*.*")])
    ...
    def use_as_input(self, what):
        if what == "v":
            ...
            self.TEXT_o_sct.configure(state="normal")
            self.TEXT_o_sct.delete("1.0", tkinter.END)
            self.TEXT_o_sct.insert(tkinter.INSERT, self.texts["v"])
            self.TEXT_o_sct.configure(state="disabled")
            self.start()
        elif what == "e":
            ...
            self.TEXT_o_sct.configure(state="normal")
            self.TEXT_o_sct.delete("1.0", tkinter.END)
            self.TEXT_o_sct.insert(tkinter.INSERT, self.texts["e"])
            self.TEXT_o_sct.configure(state="disabled")
            self.start()
