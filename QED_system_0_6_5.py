...
class Verschlüsselung():
    ...
    def structure_mix_letter(self, way:bool, key:int, full_text_:str, chunk = 4) -> str:
        """
    ...
    def structure_m1(self, way, text:str, key:list) -> str:
        ...
        if not(way):#ver
            for i in range(len(text_)-1, -1, -1): text_[i] = self.VER_1(text=text_, key=key, part=text_[i], pos=i)
        else:#ent
