...
class Verschlüsselung():
    def __init__(self, chunk = 16, debug = True, cube_field_data_size = 1, debug_c=False) -> None:
        """
    ...
    def get_key(self, KEY:str, text=""):
        ...
        if key_mix>1: key_mix = int(key_mix%len(hilfsfunktionen.BitToStr(text, self.chunk)))
        else: key_mix = int((key_mix*len(hilfsfunktionen.BitToStr(text, self.chunk)))%len(hilfsfunktionen.BitToStr(text, self.chunk)))

        key_start = int(S3[0]%len(hilfsfunktionen.BitToStr(text, self.chunk))) if S3[0]>=0 else int((S3[0]*len(hilfsfunktionen.BitToStr(text, self.chunk)))%len(hilfsfunktionen.BitToStr(text, self.chunk)))
            ...
            if i%2 == 0:
                for i2 in range(key[i]):
                    ...
                    if pos < 0:
                        go_on = False
                        break
                    part = hilfsfunktionen.IntToBit((int(part, 2)^int(text[pos][:len(part)], 2)), len(part))
            else: 
                pos-=key[i]
                if pos < 0:
                    go_on = False
    ...
    def structure_mix_letter(self, way:bool, key:int, full_text_:str) -> str:
        ...
        """
        chunk = 4 #?
        full_text = list(hilfsfunktionen.BitToStr(full_text_, chunk))
        #erzeugung und anpassung des Schlüssels + Text
        key_ = key
        go_on = True
        text_end = False
        ...
        for i in range(len(key_mix)): key_mix[i] = int(hilfsfunktionen.StrToBit(key_mix[i], lenght=chunk),2) + 1
        s2 = key_mix.copy()
        s2.sort()
        ...
        return "".join(i for i in hilfsfunktionen.StrToBit(full_text, lenght=chunk))
