...
class Verschlüsselung():
    ...
    def cube(self, text, key_m_cube, cube_dimensions=0, cube_field_data_size=0, encryption=True) -> int:
        '''
    ...
    def cube_big(self, text, key_m_cube, cube_dimensions=0, cube_field_data_size=0, encryption=True) -> int:
        '''
        mappt <text> in <cube_field_data_size> großen stücken auf die oberfläche eines rubics-cube
        mit <cube_dimensions> "flächen" (das entscheidet also obs ein 3x3x3, 4x4x4, ... ist)
        ...
        l = cube_dimensions**2 *6 *cube_field_data_size
        text_formatted = text & ((1<<(l))-1)
        text_formatted = f"{text_formatted:0{l}b}"
        ...
        text_scrambled = ((text >> l)<<l) + int(self._cube_get_data(cube.cube.copy()),2)
        return text_scrambled
